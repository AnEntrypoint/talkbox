...
class Talkbox {
    ...
    async subscribe(callback, options = {}) {
        ...
        // Offset by 2 seconds to ensure absolute isolation from previous session burials
        const startTime = (options.since || Math.floor(Date.now() / 1000)) + 2;
        const filter = {
        ...
        // Passing filter object directly - nostr-tools v2 handles the array wrapping internally
        const sub = this.pool.subscribeMany(this.relays, filter, {
            onevent(event) {
                // Ignore identical-second events to isolate from preceding session
